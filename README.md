# PID Driver

My `hello world` into the Mechatronics field!

In this project I am making a PID controlled Torque Screwdriver.

This directory is also an obsidian vault; I would recommend to open it in obsidian.

## Table of Contents

### Research
- [domain research](./notes/Domain_Research.md)
- [supply chain research](Supply_Chain_Research.md)
- [software used](./notes/Software_Used.md)

### Implementations
- [software readme](./software/README.md)
- [hardware readme](./hardware/README.md)
- [electronics readme](./electronics/README.md)
